import { Link, Outlet } from "react-router-dom";
import { CgProfile } from "react-icons/cg";
import { AiOutlineRight } from "react-icons/ai";

import { BsFillChatSquareTextFill, BsToggleOn } from "react-icons/bs";
import { MdOutlineDashboard, MdOutlineCastForEducation } from "react-icons/md";
import "./index.css";

import React from "react";
import Header from "../components/header/index";

export default function Layout() {
  return (
    <div>
      <Header />

      <div className="relative">
        <div
          className={`h-screen bg-black p-3 w-48 top-0 absolute sidebar z-50`}
        >
          <div className="flex flex-row justify-center">
            <img
              className="h-20 w-20 rounded-full"
              src="data:image/jpeg;base64,"
            />
          </div>
          <div className="mt-4 flex flex-row justify-center ">
            <button
              href="#"
              class="text-gray-100 bg-[#33de33]  rounded-md w-[70%]  text-center py-2.5 text-sm font-semibold hover:text-purple-600 "
            >
              Edit Profile
            </button>
          </div>

          {/* Search */}
          <h1 className=" text-gray-100  text-md mt-4 bg-[#252626]  p-2 px-4">
            General
          </h1>

          <ul className="pt-2">
            <Link
              to="/"
              className="rounded-md text-gray-100  text-xs flex items-center gap-x-4 cursor-pointer hover:bg-gray-50 hover:text-blue-500  p-2"
            >
              <span className="text-xl block">
                <CgProfile />
              </span>
              <span className={` text-xs font-md flex-1 hidden-link `}>
                Dark Mode
              </span>
              <BsToggleOn className="text-[#33de33] text-2xl" />
            </Link>
            <Link
              to="/marketplace"
              className="rounded-md text-gray-100  text-xs flex items-center gap-x-4 cursor-pointer hover:bg-gray-50 hover:text-blue-500  p-2"
            >
              <span className="text-xl block">
                <MdOutlineDashboard />
              </span>
              <span className={` text-sm font-md flex-1 hidden-link`}>
                Help Center
              </span>
              <AiOutlineRight />
            </Link>

            <Link
              to="/chat"
              className="rounded-md text-gray-100  text-xs flex items-center gap-x-4 cursor-pointer hover:bg-gray-50 hover:text-blue-500  p-2"
            >
              <span className="text-lg block">
                <BsFillChatSquareTextFill />
              </span>
              <span className={` text-xs font-md flex-1  hidden-link`}>
                Language
              </span>

              <AiOutlineRight />
            </Link>

            <Link
              to="/chat"
              className="rounded-md text-gray-100  text-xs flex items-center gap-x-4 cursor-pointer hover:bg-gray-50 hover:text-blue-500  p-2"
            >
              <span className="text-lg block">
                <BsFillChatSquareTextFill />
              </span>
              <span className={` text-xs font-md flex-1  hidden-link`}>
                Address
              </span>

              <AiOutlineRight />
            </Link>

            {/* Orders */}
            <Link
              to="/education"
              className="rounded-md text-gray-100  text-xs flex items-center gap-x-4 cursor-pointer hover:bg-gray-50 hover:text-blue-500  p-2"
            >
              <span className="text-xl block">
                <MdOutlineCastForEducation />
              </span>
              <span className={` text-xs font-md flex-1  hidden-link `}>
                Favourite
              </span>

              <AiOutlineRight />
            </Link>
          </ul>

          <h1 className=" text-gray-100  text-md mt-4 bg-[#252626]  p-2 px-4">
            About App
          </h1>

          <ul className="pt-2">
            <Link
              to="/"
              className="rounded-md text-gray-100  text-xs flex items-center gap-x-4 cursor-pointer hover:bg-gray-50 hover:text-blue-500  p-2"
            >
              <span className="text-xl block">
                <CgProfile />
              </span>
              <span className={` text-xs font-md flex-1 hidden-link `}>
                Privacy Policy
              </span>
            </Link>
            {/* Dashboard */}
            <Link
              to="/marketplace"
              className="rounded-md text-gray-100  text-xs flex items-center gap-x-4 cursor-pointer hover:bg-gray-50 hover:text-blue-500  p-2"
            >
              <span className="text-xl block">
                <MdOutlineDashboard />
              </span>
              <span className={` text-sm font-md flex-1 hidden-link`}>
                Legal Notices
              </span>
            </Link>

            {/* Lens */}
            <Link
              to="/chat"
              className="rounded-md text-gray-100  text-xs flex items-center gap-x-4 cursor-pointer hover:bg-gray-50 hover:text-blue-500  p-2"
            >
              <span className="text-lg block">
                <BsFillChatSquareTextFill />
              </span>
              <span className={` text-xs font-md flex-1  hidden-link`}>
                About
              </span>
            </Link>

            <Link
              to="/chat"
              className="rounded-md text-gray-100  text-xs flex items-center gap-x-4 cursor-pointer hover:bg-gray-50 hover:text-blue-500  p-2"
            >
              <span className="text-lg block">
                <BsFillChatSquareTextFill />
              </span>
              <span className={` text-xs font-md flex-1  hidden-link`}>
                Terms & Conditions
              </span>
            </Link>

            {/* Orders */}
            <Link
              to="/education"
              className="rounded-md text-gray-100  text-xs flex items-center gap-x-4 cursor-pointer hover:bg-gray-50 hover:text-blue-500  p-2"
            >
              <span className="text-xl block">
                <MdOutlineCastForEducation />
              </span>
              <span className={` text-xs font-md flex-1  hidden-link `}>
                Rate Us
              </span>
            </Link>
          </ul>

          <div className="mt-4 flex flex-row justify-center ">
            <button
              href="#"
              class="text-gray-100 bg-[#33de33]  rounded-md w-full  text-center py-2.5 text-sm font-semibold "
            >
              Logout
            </button>
          </div>
        </div>
      </div>

      <div className="ml-44">
        <Outlet />
      </div>
    </div>
  );
}
